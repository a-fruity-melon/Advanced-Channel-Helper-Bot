...
import urllib.request, urllib.parse, sys, json, os
import helper_const
...
def update_comments(bot, channel_id, msg_id, update_mode):
    ...
    with open(sys.path[0] + "/helper_const.py", "r") as f:
        lines = f.readlines()
        for line in lines:
            if line.strip().startswith("BOT_TOKEN") or line.strip().startswith("FILES_GROUP"):
                # trusted file
                exec(line, scope)
    ...
    for record in rev_records:
        ...
        if fid != "":
            ...
            with urllib.request.urlopen(ctx) as resp:
                ...
                url += "bot" + scope["BOT_TOKEN"] + "/" + json.loads(result)["result"]["file_path"]
        info = {
            "name": name,
            "type": ftype,
            "content": content,
...
def private_msg(bot, update):
    ...
    if not comment_exist:
        ...
        comment_message = bot.send_photo(
            chat_id=channel_id,
            photo=open(os.path.abspath(os.path.dirname(sys.argv[0]) + "/default.jpg"), "rb"),
